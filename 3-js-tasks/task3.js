...
function second(e, array){
    ...
    return new Promise((resolve, reject) => {
        ...
        if (typeof obj2[array2][0] === "number") {
            const sum = obj2[array2].reduce((a, b) => a + b, 0);
            sum > 35 ? resolve(`Resolved (Number): Sum = ${sum}`) 
                     : reject("Rejected: Sum <= 35");
        }

        
        else if (typeof obj2[array2][0] === "string") {
            const allValid = obj2[array2].every(str => str.length > 3);
            allValid ? resolve("Resolved (String): All substrings > 3") 
                     : reject("Rejected: Some strings length <= 3");
        }

        
        else if (typeof obj2[array2][0] === "boolean") {
            const trueCount = obj2[array2].filter(v => v === true).length;
            const falseCount = obj2[array2].filter(v => v === false).length;
